...
# Window setup, color definitions, and the Spot class remain unchanged.
# Making Window
WIDTH = 800
...
# Second algorithm
def dijkstrasAlgo(draw, grid, start, end):
    count = 0
    open_set = PriorityQueue()
    open_set.put((0, count, start))
    came_from = {}
                ...
                if start and end:
                    if event.key == pygame.K_a:
                        ...
                        for row in grid:
                            for spot in row:
                                spot.update_neighbor(grid)
                        dijkstrasAlgo(lambda: draw(win, grid, ROWS, width), grid, start, end)
    pygame.quit()
